...
main() {
    ...
    info "Install jazzy"
    local gemfile_path="$SCRIPT_DIR/Gemfile"
    ...
    local jazzy_config_path="$SCRIPT_DIR/.jazzy.yaml"
    run_jazzy "$gemfile_path" "$jazzy_config_path" "$DOCS_OUTPUT"
...
pre_check() {
    ...
    # Check gh has write auth – mbx-ci github writer token
}
